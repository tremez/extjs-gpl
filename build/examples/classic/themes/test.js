...
Ext.define('Ext.ux.event.Player', {
    ...
    snap: function() {
        ...
        if (x && x.poll) {
            x.poll.sendSyncRequest({ cmd: 'screenshot' });
        }
        else {
            alert('sreenshot');
        }
    ...
    onStop: function() {
        ...
        if (x && x.poll) {
            x.poll.sendSyncRequest({ cmd: 'finish' });
        }
    },
...
Ext.onReady(function() {
    if (!window.__x) {
        ...
        window.__x.poll.interval = 50;
        window.__x.poll.connect();
    }

    //        
    //    
    window.__x.player = Ext.create('Ext.ux.event.Player', {
        eventQueue: [{
